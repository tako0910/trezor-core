...
def validate_full_path(
    ...
    if not validate_purpose(path[0], coin, script_type):
        return False
...
def validate_purpose(
    purpose: int, coin: CoinInfo, script_type: InputScriptType
) -> bool:
    if purpose not in [
        44 | HARDENED,
        45 | HARDENED,
        ...
        return False
    if not coin.segwit and purpose not in [44 | HARDENED, 45 | HARDENED]:
        return False

    if purpose == 44 | HARDENED and script_type != InputScriptType.SPENDADDRESS:
        return False
    if (
        purpose in [45 | HARDENED, 48 | HARDENED]
